    ...
    <div class="row justify-content-center">
        ...
        <div class="col-md-12">
            ...
            <div class="d-flex align-items-center p-3 mb-3 bg-{{ $colorTheme }} p-2 text-dark bg-opacity-25 rounded shadow-sm">
                <div class="me-auto">
                    <h3 class="mb-0 lh-1">Pencatatan Penjualan Sementara</h3>
                </div>
                <div class="ms-auto">
                    <a class="btn btn-{{ $colorTheme }}" href="{{ route($pageName . '.create') }}">{{ __('Tambah Pencatatan') }}</a>
                </div>
            </div>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <form method="POST" action="{{ route($pageName . '.destroy', [ 'tempnote' => $row->id ]) }}">
                                        ...
                                        @if ($row->inventory_id)
                                            <a href="{{ route('penjualan.edit', ['penjualan' => $row->inventory_id, 'tempID' => $row->id]) }}" class="btn btn-sm btn-secondary">
                                        @else
                                            <a href="{{ route('penjualan.create', ['tempID' => $row->id]) }}" class="btn btn-sm btn-secondary">
                                        @endif                                        
                                        ...
                                        &nbsp;
                                        <a href="{{ route( $pageName . '.edit', [ 'tempnote' => $row->id ]) }}" class="btn btn-sm btn-warning">
